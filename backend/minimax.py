...
def minimax_alpha_beta_DLS(grid, depth,  alpha, beta, start_time, time_limit, first_move, players_turn, do_pruning=False, two_branch_score=False):
    ...
    if players_turn: 
        ...
        for move in moves:
            ...
            child.move(move)
            if verbose: 
                print("Before move:", "depth=", depth, "alpha=", alpha, "beta=", beta, "score", "score=", heuristic(grid), "two_branch_score=", two_branch_score)
            if first_move == None:
            ...
            if do_pruning:
                if not two_branch_score:
                 ...
                 #   print("This is four branch")
                    if 0.9*two_branch_score + 0.1*score >= beta:
                        #print("PRUNING 4-branch!", "depth=", depth, "move=", move)                        
                        #pass
        ...
        for cell in cells:
            ...
            # SKAL DER VÆRE OPDATERING AF alpha eller beta her? Jeg er ikke nået dertil endnu. Se tidligere implementering
            child = grid.clone()
